...
class Command(BaseCommand):
    help = (
        "Delete all OEIS items. Guarded by a 7-day throttle; use --force to override."
    )
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--force",
            action="store_true",
            help="Clear even if the OEIS slurper ran within the last 7 days.",
        )
    ...
    def handle(self, *args, force=False, **options):
        ...
        if not force and not SlurperRun.can_run(source, MIN_INTERVAL):
            if sys.stdin.isatty():
                answer = (
                    input(
                        f"OEIS slurper ran within the last {MIN_INTERVAL.days} days. "
                        f"Clear anyway? [y/N] "
                    )
                    .strip()
                    .lower()
